-- Insuficient testata
-- O singura data lista de isntall_id-uri

 with s_install_ids as (
        select '{0}' str from dual
)
, install_ids as (        
    select to_number(regexp_substr (str, '\d+', 1, level)) id
    from   s_install_ids
    connect by level <= 
...
, acc_gwp as (
    ...
    from gwp        
    where install_id in (select id from install_ids)    
...
, pif as (        
        ...
        ,      round(pif.risk_amnt * (
                select round(sum(gl_local_amnt) / sum(risk_amnt), 4)  currency_rate
                from  insis_prem_inst_fract pifr
                where  pifr.preminst_id = pif.preminst_ref
        ...
        and pif.preminst_id in (select id from install_ids)    
